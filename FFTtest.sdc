...
#
 create_clock -name clock50 -period "50MHz" [get_ports MCLK	]
 
 #BCLK = 32bit x 2ch x 48kHz = 3.072MHz
 create_clock -period "3.072 MHz" -name clk_audbck [get_ports AUD_BCLK]
 #Left right clock, 48 khz
 create_clock -period "48 kHz" -name clk_audlrc [get_ports AUD_ADCLRCK]
 
 create_clock -period "25 MHz" -name clock25 [get_ports clock_25]
...
set_clock_groups -asynchronous -group [list clock50] \
                               -group [list clk_audbck] 
